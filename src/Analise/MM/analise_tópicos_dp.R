...
dtm_filtrada<- readRDS("/home/reinaldo/dtm_final_unicas_18_10.rds")
dim(dtm_filtrada)
...
dp_daianeGrupos_MM_DP<- readRDS(file = paste0(caminho, "Grupos_MM_DP_20_10.rds"))
...
df_id_grupo <- data.frame(
  ID_musica = as.character(row.names(dados)), # <--- CORREÇÃO AQUI as.numeric dtm_filtrada
...
#saveRDS(data_dp, file = "data_mm_dp_final.rds")

View(data_dp %>% dplyr::filter(Grupos_MM_DP == 1))

data_dp %>% filter(ID == 9396)
#=========================================================================================================
