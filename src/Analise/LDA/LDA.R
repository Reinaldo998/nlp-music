...
k_atual <- 85# <--- DEFINE AQUI O NÚMERO DE TÓPICOS QUE VOCÊ DESEJA PARA ESTA EXECUÇÃO!

# --- 3. Ajustar o Modelo LDA ---
#ap_lda_atual <- LDA(dtm_final_unicas_18_10, k = k_atual, method = "Gibbs")

ap_lda_atual <- LDA(dtm_final_unicas_18_10, 
                    k = k_atual, 
                    method = "Gibbs",
                    control = list(
                      seed = 100,    # Garante que o resultado seja o mesmo se rodar de novo
                      burnin = 5000,  # Descarta as primeiras 1000 iterações (estabilização)
                      iter = 2500,    # Total de iterações após o burn-in
                      thin = 5      # Intervalo de salto
                    ))
...
df_atribuicao_topico <- data.frame(
  Codigo = as.character(row.names(prob_documento_topico_atual)), # Pega os Códigos da DTM
...
saveRDS(data_com_topicos_atual, file = "data_LDA_final.rds")
...
topicos_mais_populosos <- head(contagem_musicas_por_topico, 10)$topico_lda
